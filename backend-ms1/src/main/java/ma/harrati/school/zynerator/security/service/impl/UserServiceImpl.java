...
import ma.harrati.school.zynerator.security.dao.facade.core.UserDao;
import ma.harrati.school.zynerator.security.dao.specification.core.UserSpecification;
import ma.harrati.school.zynerator.security.service.facade.*;
...
public class UserServiceImpl extends AbstractServiceImpl<User, UserCriteria, UserDao> implements UserService {
    ...
    private User createWithEnable(User t, boolean enable) {
        ...
        if (foundedUserByUsername != null || foundedUserByEmail != null) return null;
			...
			else if(roleAsString.equals(AuthoritiesConstants.STAFF)){
				saved = staffService.create(convertToStaff(t));
			}
 
			else if(roleAsString.equals(AuthoritiesConstants.PROFESSOR)){
				saved = professorService.create(convertToProfessor(t));
			}
 
 
			else if(roleAsString.equals(AuthoritiesConstants.PRINCIPAL)){
				saved = principalService.create(convertToPrincipal(t));
			}
	 ...
	 private Staff convertToStaff(User user) {
        Staff item = new Staff();
        BeanUtils.copyProperties(user, item);
        return item;
    }
	 private Professor convertToProfessor(User user) {
        Professor item = new Professor();
        BeanUtils.copyProperties(user, item);
        return item;
    }
	 private Principal convertToPrincipal(User user) {
